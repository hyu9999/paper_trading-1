...
from app.models.enums import OrderStatusEnum, PriceTypeEnum
...
@pytest.fixture
async def order_submitting_status(
...
@pytest.fixture
async def order_not_done_status(
    test_user_scope_func: UserInDB, db: AsyncIOMotorDatabase
):
    json = {
        **order_in_create_json,
...
@pytest.fixture
async def order_part_finished_status(
    test_user_scope_func: UserInDB, db: AsyncIOMotorDatabase
):
    json = {
        **order_in_create_json,
...
@pytest.fixture
async def order_all_finished_status(
    test_user_scope_func: UserInDB, db: AsyncIOMotorDatabase
):
    json = {
        **order_in_create_json,
...
async def order_list(
    ...
    return {
        order_submitting_status.id,
        order_not_done_status.id,
        order_part_finished_status.id,
        order_all_finished_status.id,
        order_cancel_status.id,
        order_reject_status.id,
    }
...
async def test_main_engine_can_load_entrust_orders(
    main_engine: MainEngine, mocker: MockerFixture, order_list: set
):
    mocker.patch(
        "app.services.engines.market_engine.china_a_market.ChinaAMarket.put",
        mock_put_entrust_order,
    )
...
async def test_main_engine_on_order_arrived(
    ...
    mocker.patch(
        "app.services.engines.user_engine.UserEngine.pre_trade_validation",
        mock_pre_trade_validation,
    )
    mocker.patch(
        "app.services.engines.market_engine.china_a_market.ChinaAMarket.put",
        mock_put_entrust_order,
    )
    order = await main_engine.on_order_arrived(
        OrderInCreate(**json), test_user_scope_func
    )
    await asyncio.sleep(1)
    order_after_create = await OrderRepository(db).get_order_by_entrust_id(
        order.entrust_id
    )
...
async def test_main_engine_can_refuse_order(
    ...
    await asyncio.sleep(1)
    order_after_create = await OrderRepository(db).get_order_by_entrust_id(
        order_not_done_status.entrust_id
    )
