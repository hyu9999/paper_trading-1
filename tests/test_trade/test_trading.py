...
async def main_engine_with_mock(
    db: AsyncIOMotorDatabase, quotes_api: HQ2Redis, session_mocker
):
    session_mocker.patch(
        "app.services.engines.main_engine.MainEngine.load_entrust_orders",
        mock_load_entrust_orders,
    )
...
async def test_user_buy(
    ...
    order = await main_engine_with_mock.on_order_arrived(
        order_buy, test_user_scope_func
    )
    await asyncio.sleep(2)
    # 订单
    order_after_trade = await OrderRepository(db).get_order_by_entrust_id(
        order.entrust_id
    )
    ...
    assert (
        statement.costs.commission.to_decimal()
    ...
    assert (
        ...
        == test_user_scope_func.cash.to_decimal()
        - amount
        - amount * test_user_scope_func.commission.to_decimal()
    )
    assert (
        user_after_update.assets.to_decimal()
        == user_after_update.securities.to_decimal()
        + user_after_update.cash.to_decimal()
    )

    # 持仓
    position_after_update = await PositionRepository(db).get_positions_by_user_id(
        test_user_scope_func.id
    )
    assert position_after_update
    assert (
        position_after_update[0].volume
        == order_buy.volume
        == position_after_update[0].available_volume
    )
...
async def test_user_sell(
    ...
    order = await main_engine_with_mock.on_order_arrived(
        order_sell, test_user_scope_func
    )
    await asyncio.sleep(3)
    # 订单
    order_after_trade = await OrderRepository(db).get_order_by_entrust_id(
    ...
    assert (
        statement.costs.tax.to_decimal()
        == amount * test_user_scope_func.tax_rate.to_decimal()
    )
    assert (
        statement.costs.commission.to_decimal()
        == amount * test_user_scope_func.commission.to_decimal()
    ...
    assert (
        user_after_update.cash.to_decimal()
        == user_after_buy.cash.to_decimal()
        + amount
        - statement.costs.total.to_decimal()
    )
    assert (
