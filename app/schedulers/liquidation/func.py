    ...
    for user in users:
        ...
        for position in position_list:
            ...
            dividend_flow_list = liquidate_dividend(
                dividend_detail, Position(**position.dict()), liq_date
            )
            if dividend_flow_list:
                for dividend_flow in dividend_flow_list:
                    statement = StatementInDB(
                        symbol=dividend_flow.symbol,
                        exchange=MARKET_MAPPING[dividend_flow.exchange],
                        user=user,
                        trade_category="dividend",
                        volume=dividend_flow.stkeffect,
                        solod_price="0",
                        amount=dividend_flow.fundeffect,
                        costs=no_cost,
...
async def update_user_by_flow(user: UserInCache, flow: StatementInDB) -> UserInCache:
    """通过分红流水更新账户."""
    user.cash = PyDecimal(user.cash.to_decimal() + flow.amount)
    user.available_cash = user.cash
    user.assets = PyDecimal(user.assets.to_decimal() + flow.amount)
    return user
...
async def update_position_by_flow(
    ...
    dividend_detail = DividendDetail.query_data(
        filters=[
            ...
            DividendDetail.dividend_pay_date == get_utc_now().date(),
        ]
    )
